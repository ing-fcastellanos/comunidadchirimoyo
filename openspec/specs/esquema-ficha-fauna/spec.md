...
El campo `fotos` SHALL ser un arreglo de objetos `{ archivo, credito, alt, licencia? }`, donde `archivo` es una ruta relativa a la carpeta de la ficha, `credito` y `alt` son obligatorios, y la primera entrada es la portada. El campo opcional `audios` SHALL ser un arreglo de objetos `{ archivo, credito, descripcion?, licencia? }`. Toda foto y audio SHALL declarar `credito`; el `alt` de cada foto existe por accesibilidad y queda aislado como string traducible (i18n-ready, ADR-0011).
