...
El servicio SHALL registrar los blueprints `voluntarios` (`/api/voluntarios`) y `contacto` (`/api/contacto`). En este scaffold sus handlers SHALL responder `501 Not Implemented` indicando que la lógica se implementa en Fase 4. La lógica real (validación, escritura en Firestore, email, privacidad) NO SHALL implementarse en este change.

#### Scenario: Stubs visibles
- **WHEN** se hace una petición a `/api/voluntarios` o `/api/contacto`
- **THEN** responde `501` con un mensaje que indica "pendiente Fase 4"
