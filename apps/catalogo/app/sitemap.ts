...
/* sitemap.ts — sitemap del catálogo de fauna (ADR-0024: dominio único
   ...
   `/reptiles`), el buscador de aves, la búsqueda general (`/busqueda`), la
   página de colaboradores (`/colaboradores`) y la ficha de cada especie
   existente (`/<grupo>/<slug>`). No se incluyen los
   placeholders «próximamente» de grupos
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const rutas = ["/", ...gruposConFichas, "/aves/buscador", "/busqueda", "/colaboradores"];
